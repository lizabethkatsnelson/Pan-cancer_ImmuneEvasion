...
setwd("/gpfs/data/davolilab/projects/melanoma_scrna/bulk_analysis/SKCM")
...
for (arm in arms_to_plot) {
  ...
  if (!(arm %in% colnames(SKCM_armcnv_df))) {
    message(paste("Skipping", arm, "- not found in SKCM_ArmCNV"))
    next
  }
  ...
  # Save plots
  file_base <- paste0("SKCM_", arm, "_IS_RankedSum")
